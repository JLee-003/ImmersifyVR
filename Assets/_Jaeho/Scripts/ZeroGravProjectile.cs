...
public class ZeroGravProjectile : MonoBehaviour
{
    ...
    private void Awake()
    {
        ballSpawnPosition = new Vector3(0f, 2.5f, -4.5f);
        if (playerSpawnPoint != null && CalibrationMeasurements.Instance != null)
        {
            Vector3 ballPosition = playerSpawnPoint.position;
            ballPosition.z += CalibrationMeasurements.Instance.comfortReach;
            ballPosition.y += 1;
            transform.position = ballPosition;
            ballSpawnPosition = ballPosition;
        }
    }
